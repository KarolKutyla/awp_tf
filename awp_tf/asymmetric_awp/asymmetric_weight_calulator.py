...
class WeightCalculator:
    def __init__(
        ...
        self._indices_of_selected_layers = [i for i, tracked in enumerate(self._perturbation_scales) if tracked > 0.0]
        self._saved_weights: list[tf.Variable | None] = _make_weight_perturbation_storage(self._classifier, self._perturbation_scales)
        self._weight_perturbations: list[tf.Variable | None] = _make_weight_perturbation_storage(self._classifier, self._perturbation_scales)
        self._weight_norms: list[tf.Variable | None] = _make_weight_norms_storage(self._classifier, self._perturbation_scales)
    ...
    def _calculate_weight_perturbation_body(self, i, x_pert, y_batch):
    ...
    def _scale_perturbation_to_bound(self, weight_perturbation: tf.Tensor, idx) -> tf.Tensor:
        allowed_norm = (
                self._weight_norms[idx]
                * self._weight_constraint
                * self._perturbation_scales[idx]
        )
        scale_factor = (
...
def _make_weight_perturbation_storage(classifier: tf.keras.models.Model, perturbed_layers: tuple[float, ...]) -> list[tf.Variable | None]:
    return [tf.Variable(tf.zeros_like(variable), trainable=False) if perturbed > 0.0 else None
            for variable, perturbed in zip(classifier.trainable_weights, perturbed_layers)]


def _make_weight_norms_storage(classifier: tf.keras.models.Model, perturbed_layers: tuple[float, ...]) -> list[tf.Variable | None]:
    return [tf.Variable(tf.norm(variables), trainable=False) if perturbed > 0.0 else None
