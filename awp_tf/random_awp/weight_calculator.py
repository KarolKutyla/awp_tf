...
class WeightCalculator:
    ...
    def _calculate_perturbation_for_single_trainable_variable(self, weight_gradient: tf.Tensor, idx) -> tf.Tensor:
        random_direction = tf.random.normal(
            shape=tf.shape(self._weight_perturbations[idx]),
            mean=0.0,
            stddev=1.0,
            dtype=self._weight_perturbations[idx].dtype,
        )

        weight_perturbation = random_direction * self.step_size * self._weight_norms[idx]
