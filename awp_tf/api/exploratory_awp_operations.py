...
class Calculator:
    ...
    def calculate_perturbation_for_smooth_params(self, gradients: tuple[tf.Tensor, ...]) -> None:
    ...
    def calculate_perturbation_for_steep_params(self, gradients: tuple[tf.Tensor, ...]) -> None:
        for idx, gradient, perturbation, norm in zip(
                self._applied_layers, gradients, self._weight_perturbations, self._weight_norms
        ):
            gradient_norm = tf.abs(gradient)
