...
class EpochLogger(tf.keras.callbacks.Callback):
    ...
    def _register_epoch(self, epoch: int, logs: dict):
        ...
        with open(self._save_filepath, "a") as log_file:
            log_file.write(f"{datetime_prefix} - {epoch}:\t{logs.get("train_time")}\t{logs.get("val_time")}\t{logs.get("lr")}\t"
                           f"{logs.get("loss")}\t{logs.get("accuracy")}\t{logs.get("robust_loss")}\t{logs.get("robust_accuracy")}\t"
                           f"{logs.get("val_loss")}\t{logs.get("val_accuracy")}\t{logs.get("val_robust_loss")}\t{logs.get("val_robust_accuracy")}\n")
